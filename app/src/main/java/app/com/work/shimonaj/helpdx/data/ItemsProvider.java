...
public class ItemsProvider extends ContentProvider {
	...
	public String getType(Uri uri) {
		...
		switch (match) {
			...
			case COMMENTS:
				return ItemsContract.Comments.CONTENT_ITEM_TYPE;
			default:
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch (match) {
			...
			case COMMENTS: {
				...
				getContext().getContentResolver().notifyChange(uri, null);
				return ItemsContract.Comments.buildCommentUri(_id);
			}
			default: {
				throw new UnsupportedOperationException("Unknown uri: " + uri);
			}
	...
	private SelectionBuilder buildSelection(Uri uri, int match, SelectionBuilder builder) {
		...
		switch (match) {
			...
			case ITEMS__ID: {
				...
				return builder.table(Tables.ITEMS).where(ItemsContract.Items.TICKETID + "=?", _id);
			}
			case COMMENTS: {
				final String _id = paths.get(1);
				return builder.table(Tables.COMMENTS).where(ItemsContract.Items.TICKETID + "=?", _id);
			}
			default: {
				throw new UnsupportedOperationException("Unknown uri: " + uri);
			}
